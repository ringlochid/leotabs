...
const names={keep:'Keep existing',rules:'Built-in grouping',ai:'AI','rules-ai':'Built-in grouping, then AI',template:'Template',manual:'Keep current order',title:'Alphabetical',domain:'Website',recent:'Recent activity',rule:'Rule priority'};
...
export function organisationDialog({state,scope={type:'global'},change}) {
  ...
  const form=el('div',{class:'organisation-fields'},...Object.entries(POLICY_CHOICES).filter(([key])=>scope.type!=='collection'||key!=='collectionOrder').map(([key,values])=>{
    const input=fields[key]=el('select',{'aria-label':labels[key]},...values.filter(value=>value!=='rule').map(value=>el('option',{value,selected:value===policy[key]},names[value])));
  ...
  const {close}=modal('Organisation · '+(scope.type==='global'?'Global defaults':target.name),body,[
    ...
    button('Save',task(async()=>{
      ...
      await change('organisation-policy',{scope,organisation});close();
    }),{className:'primary'}),
  ]);
}
