(() => {
  ...
  var names = { keep: "Keep existing", rules: "Built-in grouping", ai: "AI", "rules-ai": "Built-in grouping, then AI", template: "Template", manual: "Keep current order", title: "Alphabetical", domain: "Website", recent: "Recent activity", rule: "Rule priority" };
  ...
  function organisationDialog({ state, scope = { type: "global" }, change }) {
    ...
    const { close } = modal("Organisation \xB7 " + (scope.type === "global" ? "Global defaults" : target.name), body, [
      ...
      button("Save", task(async () => {
        ...
        await change("organisation-policy", { scope, organisation });
        close();
      }), { className: "primary" })
    ]);
  }
