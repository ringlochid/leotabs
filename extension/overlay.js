(() => {
  ...
  var DEFAULT_RULES = [{ id: "github", domain: "github.com/*", group: "Github" }];
  ...
  function toast(message, { undo, error = false } = {}) {
    ...
    if (!error) node._timer = setTimeout(() => node.hidden = true, 5e3);
  }
  function task(fn) {
    return async (event) => {
      try {
  ...
  function createActionDialogs({ getData, windowId, getTabIds, change, onOpen = () => {
    ...
    async function aiTabs() {
      const context2 = await rpc("ai-tabs-context", { windowId: win, tabIds: selectedIds() });
      if (!context2.collection.links.length) return toast("No pages to organise.");
      aiDialog(context2.collection, { liveContext: context2 });
    }
    ...
    function reviewPlan(c, plan, liveContext) {
      ...
      for (const link of c.links.filter((l) => plan.scopeLinkIds.includes(l.id))) {
        ...
        const picker = el("select", { "aria-label": "Group for " + link.title, onchange: (e) => {
          for (const g of plan.groups) g.linkIds = g.linkIds.filter((id) => id !== link.id);
          const destination = plan.groups.find((g) => g.id === e.target.value);
          if (destination) destination.linkIds.push(link.id);
          updatePreview();
        } }, el("option", { value: "" }, "Leave unchanged"), ...plan.groups.map((g) => el("option", { value: g.id, selected: g === initial }, g.name)));
  ...
  function createTabTools({ getTabs, getSettings, change, actions, compact = false }) {
    const sort = button("Arrange tabs", (e) => menu("Arrange tabs", [
      ...
      ["Configure rules", () => actions.ruleSettings(), "settings"],
      null,
      ...[["position", "Browser order"], ["recent", "Most recent first"], ["title", "Title A\u2013Z"], ["domain", "Website"], ["reverse", "Reverse browser order"]].map(([tabSort, label]) => [label, () => change("settings", { settings: { tabSort } }), getSettings().tabSort === tabSort ? "check" : void 0])
