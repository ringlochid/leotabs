...
**Neo 0.13 makes grouping immediate.** Group & sort applies local rules and website defaults, short names, varied persistent colours and alphabetical order, with browser-layout Undo. Its compact Group panel remembers whether to include already grouped tabs. Auto-group uses a visible On/Off switch and waits for two matching tabs before creating a group. Open-tab groups accept drag-and-drop moves and Ungroup drops. Rules is directly visible beside Open tabs and offers presets, live match counts and rules from tabs/groups. AI topic grouping asks whether to include already grouped tabs, then applies a single batch; Organise collection with AI combines a name, note and topic groups, with no singleton groups; AI organisation is limited to open tabs or one collection. Web Store pages are normal resources. See [0.13 acceptance and performance](docs/grouping-0.13.md).

**Neo 0.12 adds configurable organisation and contextual AI.** Global policies can be overridden per space or collection. Grouping, naming and ordering are separate; URL/title rules support priorities, exceptions and colours. Optional automatic AI groups unmatched pages, names generic work and applies a requested reading order. Manual corrections are remembered. Space-wide plans review moves, merges and names before applying them. Save/drop suggestions, inline alternative names, source-based research drafts, action-based Timeline events, `neo` address-bar access and an optional Library window complete the workflow. See [implementation and validation](docs/remaining-work.md).
...
- Stash offers a new or existing destination, optional AI naming/filing suggestions and “and close them.” Closing pauses the affected current collection first. Dragging still saves directly; contextual suggestions remain optional afterward. The library and overlay share the save flow.
